...
def stage_build():
    ...
    ccache -M 50G
    timeout 95m bash -c 'source build/envsetup.sh && export KBUILD_BUILD_USER={GH_USERNAME} && export BUILD_USERNAME={GH_USERNAME} && export KBUILD_BUILD_HOST={BUILD_HOSTNAME} && export BUILD_HOSTNAME={BUILD_HOSTNAME} && export TZ=Asia/Jakarta &&  lunch lineage_{DEVICE_CODENAME}-user && make bacon'
    """
...
def stage_upload():
    ...
    zip_file_list = glob.glob(f"out/target/product/{DEVICE_CODENAME}/lineage-*.zip")
